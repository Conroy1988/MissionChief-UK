...
def synchronized_content(record: dict[str, Any], official: dict[str, Any], mappings: dict[str, dict[str, Any]]) -> str:
    expected = build_expected_patient(official, mappings)
    output = dict(record)
    if expected:
        output["patients"] = expected
    else:
        output.pop("patients", None)
...
def synchronize(check_only: bool) -> tuple[int, list[str]]:
    ...
    for path in sorted(CANONICAL_ROOT.glob("*.json")):
        ...
        if official is None:
            continue
        direct_records += 1
        content = synchronized_content(record, official, mappings)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Synchronize canonical patient fields from the retained official UK source")
    parser.add_argument("--check", action="store_true", help="Fail instead of writing when canonical patient fields differ")
    args = parser.parse_args()

    try:
        direct_records, changed = synchronize(args.check)
    except ValueError as exc:
        print(f"Canonical patient synchronization failed: {exc}", file=sys.stderr)
        return 1

    if args.check and changed:
        print("Canonical patient synchronization check failed; stale files:", file=sys.stderr)
        for path in changed:
    ...
    print(
        f"Canonical patient fields {action}: {direct_records} direct canonical records, "
