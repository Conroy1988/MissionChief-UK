...
MAPPING = {
    ...
    "canonical_id": "lifeboat_stations",
    "checked_at": "2026-07-23",
    "sources": [
        "https://www.missionchief.co.uk/einsaetze/544",
        "https://www.missionchief.co.uk/einsaetze/562",
        "https://www.missionchief.co.uk/einsaetze/563",
        "https://www.missionchief.co.uk/einsaetze/564",
        "https://www.missionchief.co.uk/einsaetze/569",
        "https://www.missionchief.co.uk/einsaetze.json"
    ],
    "notes": "The official coastal_rescue_count prerequisite is the countable Lifeboat Station mission-generation requirement. It remains distinct from water_rescue_2, which represents Coastguard Rescue Stations."
}
...
def synchronized_document() -> dict[str, Any]:
    ...
    output_prerequisites["coastal_rescue_count"] = MAPPING
    output["prerequisites"] = output_prerequisites
    return output


def main() -> int:
    parser = argparse.ArgumentParser(description="Synchronize the verified Lifeboat Station prerequisite mapping")
    ...
    try:
        ...
        print(f"Lifeboat Station mapping synchronization failed: {exc}", file=sys.stderr)
        return 1

    changed = current != content
    if args.check and changed:
        print("Lifeboat Station mapping synchronization check failed: registry is stale", file=sys.stderr)
        return 1
    if changed and not args.check:
        MAPPING_PATH.write_text(content, encoding="utf-8")
    print(
        "Lifeboat Station mapping "
