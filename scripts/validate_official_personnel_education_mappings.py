...
def audit() -> dict[str, int]:
    ...
    for mission_id, decision in decisions.items():
        ...
        if expected:
            personnel_missions += 1
        if decision.get("strict_personnel_education_equivalence") is True:
            strict_missions += 1
        if validate_promoted_personnel_educations(key, decision, official, canonical, registry):
            exact += 1

    return {
        "promoted": promoted,
        "exact": exact,
        "personnel_missions": personnel_missions,
...
def main() -> int:
    try:
        ...
        print(f"Official personnel education mapping audit failed: {exc}", file=sys.stderr)
        return 1

    print(
        "Official personnel education mapping audit passed: "
        f"{result['promoted']} promoted missions inspected, "
        f"{result['exact']} exact personnel-education missions, "
        f"{result['strict_missions']} strict decisions and "
        f"{result['roles']} mapped roles."
    )
    return 0
