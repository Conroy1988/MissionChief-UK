...
REQUIREMENT_MAPPING = {
    ...
    "sources": [
        "https://www.missionchief.co.uk/einsaetze/69",
        "https://www.missionchief.co.uk/einsaetze/73",
    ...
    "notes": "The official traffic_car count represents Traffic Cars. Missions carrying need_traffic_car_only_if_present=true are delegated to the conditional resource contract; all other records use the standard guaranteed or same-key probabilistic mapping."
}

CHANCE_MAPPING = {
    "status": "verified",
    "canonical_target": "requirements.probabilistic",
    "canonical_id": "traffic_car",
    "requirement_key": "traffic_car",
    "checked_at": "2026-07-23",
    "sources": [
        "https://www.missionchief.co.uk/einsaetze/29",
        "https://www.missionchief.co.uk/einsaetze/90",
        "https://www.missionchief.co.uk/einsaetze/587",
        "https://www.missionchief.co.uk/einsaetze.json"
    ],
    "notes": "The same-key percentage is the Traffic Car requirement probability. Conditional availability remains separately retained when the official flag is true."
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Synchronize the standard Traffic Car key mapping")
    ...
    try:
        ...
        print(f"Traffic Car mapping synchronization failed: {exc}", file=sys.stderr)
        return 1

    changed = current != content
    if args.check and changed:
        print("Traffic Car mapping synchronization check failed: registry is stale", file=sys.stderr)
        return 1
