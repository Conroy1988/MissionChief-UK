...
def synchronize(check_only: bool) -> tuple[int, int]:
    ...
    for path in sorted(CANONICAL_ROOT.glob("*.json")):
        ...
        synchronized = merge_operational_fields(canonical, expected)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Synchronize canonical operational metadata fields")
    parser.add_argument("--check", action="store_true", help="Fail instead of writing when direct records are stale")
    args = parser.parse_args()
    try:
        direct, changed = synchronize(args.check)
    except ValueError as exc:
        print(f"Canonical operational metadata synchronization failed: {exc}", file=sys.stderr)
        return 1
    if args.check and changed:
        print(
            f"Canonical operational metadata synchronization check failed: {changed} of {direct} direct records are stale",
            file=sys.stderr,
        )
        return 1
    print(
        "Canonical operational metadata "
