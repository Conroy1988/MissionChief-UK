...
ALTERNATIVE_MAPPING = {
    ...
    "canonical_ids": ["police_car", "armed_response_vehicle"],
    "checked_at": "2026-07-23",
    "sources": [
        "https://www.missionchief.co.uk/einsaetze/408",
        "https://www.missionchief.co.uk/einsaetze/410",
        "https://www.missionchief.co.uk/einsaetze/454",
        "https://www.missionchief.co.uk/einsaetze.json",
    ],
    "notes": "The official field is displayed as Police Cars or Armed Response Vehicles (ARVs). The quantity is a qualifying total across the two canonical resource types.",
}
...
def synchronized_document() -> dict[str, Any]:
    ...
    output_requirements["oneof_police_patrol_or_swat"] = ALTERNATIVE_MAPPING
    output["requirements"] = output_requirements
    return output


def main() -> int:
    parser = argparse.ArgumentParser(description="Synchronize the Police Car or ARV alternative mapping")
    ...
    try:
        ...
        print(f"Police Car or ARV mapping synchronization failed: {exc}", file=sys.stderr)
        return 1

    changed = current != content
    if args.check and changed:
        print("Police Car or ARV mapping synchronization check failed: registry is stale", file=sys.stderr)
        return 1
    if changed and not args.check:
        MAPPING_PATH.write_text(content, encoding="utf-8")
    print(
        "Police Car or ARV mapping "
